...
const articleReaction = async (
  id: number,
): Promise<ArticleReactionResponse> => {
  const res = await apiClient.post<ArticleReactionResponse>(
    `${ARTICLE_REACTION_API_URL}/${id}`,
  );

  return res.data;
};

const commentReaction = async ({
  articleId,
  commentId,
}: CommentReactionRequest): Promise<CommentReactionResponse> => {
  const res = await apiClient.post(
    `${ARTICLE_REACTION_API_URL}/${articleId}/comments/${commentId}`,
  );
